...
| E4 | Timeout enforced across all workload types | End-to-end verification that the configured waitForPodsReady timeout is enforced for every supported workload type, through the operator-managed path. | Proposed |
